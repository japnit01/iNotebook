...
router.post('/createuser', [
    ...
    try {
        ...
        if (user)
            return res.status(404).json({success, error: "Sorry!! A user with this email id already exists" })
...
router.post('/login', [
    ...
    try {
        ...
        let user = await User.findOne({ email: req.body.email });
        if (!user)
            return res.status(404).json({success, error: "Wrong email or password" })
